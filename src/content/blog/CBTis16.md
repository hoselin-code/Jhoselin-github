---
title: "Técnico en Electricidad - CBTis No. 16"
description: "Obtención del título de Técnico en Electricidad tras concluir el bachillerato tecnológico en el Centro de Bachillerato Tecnológico Industrial y de Servicios No. 16."
pubDate: 2019-07-03
readingTime: "3 min"
tags: ["Formación Técnica", "Electricidad", "CBTis"]
---

# Técnico en Electricidad

Conclusión de la educación media superior en el **CBTis No. 16** como **Técnico en Electricidad**. Esta etapa formativa fue fundamental para el desarrollo de habilidades prácticas en el manejo de circuitos eléctricos, instalaciones residenciales e industriales y mantenimiento de sistemas electromecánicos.

Esta formación técnica me proporcionó las herramientas iniciales para incursionar posteriormente en el mundo de la electrónica, los microcontroladores (como ESP32) y la integración de hardware con software, complementando mi perfil profesional actual.

<div class="my-10 group">
  <a 
    href="https://drive.google.com/file/d/19CBx6iC55qKtMpBo5n0NnlkdX43gO9On/view?usp=sharing" 
    target="_blank" 
    rel="noopener noreferrer"
    class="relative block overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-md transition-all duration-300 ease-out hover:-translate-y-2 hover:shadow-2xl hover:border-zinc-400 dark:hover:border-zinc-600"
  >
    <div class="flex flex-col sm:flex-row">
      <div class="relative h-56 sm:h-auto sm:w-1/3 overflow-hidden bg-zinc-100 dark:bg-zinc-800">
        <img 
          src="/drive.svg" 
          alt="Reconocimiento de CBTIS" 
          class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
        />
        <div class="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/10 transition-colors">
          <div class="rounded-full bg-white/90 p-3 shadow-lg group-hover:scale-110 transition-transform">
            <svg class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
              <path d="M15.832 12.017a3.85 3.96 0 0 1-3.85 3.96 3.85 3.96 0 0 1-3.848-3.96 3.85 3.96 0 0 1 3.849-3.96 3.85 3.96 0 0 1 3.849 3.96zM11.999 3.47a.723.723 0 0 0-.675.467A8.096 8.096 0 0 0 3.904 12a8.096 8.096 0 0 0 7.42 8.063.723.723 0 0 0 .676.467.723.723 0 0 0 .676-.468A8.096 8.096 0 0 0 20.096 12a8.096 8.096 0 0 0-7.421-8.064.723.723 0 0 0-.675-.467zm.648 1.044A7.518 7.518 0 0 1 19.517 12a7.518 7.518 0 0 1-6.871 7.485.723.723 0 0 0-.647-.401.723.723 0 0 0-.647.401A7.518 7.518 0 0 1 4.482 12a7.518 7.518 0 0 1 6.871-7.486.723.723 0 0 0 .646.401.723.723 0 0 0 .648-.401zM12 0a.723.723 0 0 0-.676.467A11.566 11.566 0 0 0 4.33 3.384a.723.723 0 0 0-.283-.06.723.723 0 0 0-.723.724.723.723 0 0 0 .056.28 11.566 11.566 0 0 0-2.913 6.996A.723.723 0 0 0 0 12a.723.723 0 0 0 .468.676 11.566 11.566 0 0 0 2.907 7.012.723.723 0 0 0-.05.263.723.723 0 0 0 .723.723.723.723 0 0 0 .265-.05 11.566 11.566 0 0 0 7.015 2.918.723.723 0 0 0 .672.457.723.723 0 0 0 .676-.467 11.566 11.566 0 0 0 7.012-2.907.723.723 0 0 0 .263.05.723.723 0 0 0 .723-.724.723.723 0 0 0-.05-.265 11.566 11.566 0 0 0 2.918-7.015A.723.723 0 0 0 24 12a.723.723 0 0 0-.468-.676 11.566 11.566 0 0 0-2.907-7.012.723.723 0 0 0 .05-.263.723.723 0 0 0-.724-.723.723.723 0 0 0-.265.05A11.566 11.566 0 0 0 12.671.458.723.723 0 0 0 12-.001zm-.652 1.035a.723.723 0 0 0 .652.41.723.723 0 0 0 .647-.401 10.988 10.988 0 0 1 6.632 2.739.723.723 0 0 0-.05.265.723.723 0 0 0 .722.723.723.723 0 0 0 .262-.05 10.988 10.988 0 0 1 2.751 6.628.723.723 0 0 0-.41.65.723.723 0 0 0 .401.648 10.988 10.988 0 0 1-2.738 6.632.723.723 0 0 0-.266-.05.723.723 0 0 0-.723.722.723.723 0 0 0 .05.262 10.988 10.988 0 0 1-6.627 2.752.723.723 0 0 0-.651-.411.723.723 0 0 0-.648.401 10.988 10.988 0 0 1-6.632-2.738.723.723 0 0 0 .05-.265.723.723 0 0 0-.722-.723.723.723 0 0 0-.262.05 10.988 10.988 0 0 1-2.742-6.633.723.723 0 0 0 .401-.646.723.723 0 0 0-.4-.647 10.988 10.988 0 0 1 2.752-6.628.723.723 0 0 0 .25.046.723.723 0 0 0 .724-.723.723.723 0 0 0-.049-.257 10.988 10.988 0 0 1 6.626-2.756z"/>
            </svg>
          </div>
        </div>
      </div>
      <div class="p-8 sm:w-2/3 flex flex-col justify-center">
        <div class="flex items-center space-x-2 mb-3">
          <span class="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-white bg-blue-600 rounded-md">Reconocimiento</span>
          <span class="text-xs font-medium text-zinc-500">CBTIS 2019</span>
        </div>
        <h3 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100 mb-2"> Reconocimiento </h3>
        <p class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed"> Accede al reconocimeinto otorgado por CBTis No. 16. <strong>CBTIS</strong>.
        </p>
        <div class="mt-6 flex items-center text-sm font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-blue-600 transition-colors">
          <span>Ver Documento</span>
          <svg class="ml-2 w-4 h-4 transition-transform group-hover:translate-x-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
          </svg>
        </div>
      </div>
    </div>
  </a>
</div>
